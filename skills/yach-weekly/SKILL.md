...
description: >
  知音楼周报能力。仅在以下场景使用：
  用户明确提到"周报"或"weekly"，且意图是：查询/列出周报记录、读取自己的周报草稿、保存或修改周报草稿、统计团队周报数据。
  不应因用户提到"上周工作""本周计划""工作总结"等泛化表述就触发；必须有"周报"关键词且有明确的读取或编辑意图。
---
...
5. **图片和附件处理**
   - **链接完整性**：确保所有文件附件和图片的链接正确返回，不得丢失 URL
   - **按需描述**：仅当用户明确要求描述图片内容时，才对图片进行详细说明；否则只返回链接，不主动展开图片内容
   - **隐私优先**：截图、图表等视觉内容可能含有敏感工作数据，描述前须确认用户有权查看该周报的完整内容
   - **他人周报**：查看他人附件时，遵循最小化原则，默认仅列出附件名称和链接，不主动描述内容
