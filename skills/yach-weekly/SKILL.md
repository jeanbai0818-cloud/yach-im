...
- **尽可能返回全量内容**：查询周报时应尽可能返回完整的周报数据，包括但不限于：
  - 完整的周报事项列表（不截断或省略）
  - 详细的工作描述内容
  - 所有关联的OKR信息（kr_id、object_id、okr_title）
  - **完整的附件信息（含图片、文件等所有附件，不得省略）**
  - 准确的统计数据（read_count、like_count、is_read）
- **避免内容失真**：不要对周报内容进行摘要或截断，保持原始信息的完整性
- **图片处理**：
  - **必须包含所有图片信息**：周报中的图片、截图、图表等视觉信息必须完整呈现
  - **图片描述**：对于图片内容，应尽可能详细描述图片中的信息，包括文字、数据、图表等可见内容
  - **附件链接**：确保所有附件的下载链接或访问地址正确返回
  - **不得省略**：严禁因为格式转换或内容处理而省略图片信息
- **分页处理**：如果数据量较大导致需要分页，应明确告知用户并提供继续翻页的方式，但尽可能单次返回更多数据
- **格式保持**：返回的周报内容应保持原有格式和结构，便于用户理解
