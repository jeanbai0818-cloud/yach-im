 ...
 *
 * 所有接口使用 app_access_token（Authorization: Bearer），form-urlencoded 提交。
 */
...
export class YachGroupApi {
    ...
    async createGroup(params) {
        ...
        const members = params.memberUserIds?.length
            ...
            : params.ownerUserId;
        const res = await postForm(`${this.baseUrl}/group/create`, token, { group_name: params.name, group_owner: params.ownerUserId, group_userids: members });
    ...
    async addMembers(groupId, userIds, opUid) {
        const token = await this.getToken();
        const res = await postForm(`${this.baseUrl}/group/users/add`, token, { group_tid: groupId, userid_list: JSON.stringify(userIds), op_uid: opUid });
        const data = (await res.json());
        if (!isOk(data.code))
    ...
    async removeMembers(groupId, userIds, opUid) {
        const token = await this.getToken();
        const res = await postForm(`${this.baseUrl}/group/users/del`, token, { group_tid: groupId, userid_list: JSON.stringify(userIds), op_uid: opUid });
        const data = (await res.json());
        if (!isOk(data.code))
    ...
    async listMembers(groupId) {
        const token = await this.getToken();
        const res = await postForm(`${this.baseUrl}/group/users/list`, token, { group_tid: groupId });
        const data = (await res.json());
        if (!isOk(data.code) || !data.obj) {
